...
```{r}
notinboth <- setdiff(Animals2, mammals)
#Los 3 animales que no están son el diplodocus, el triceratops y el braquiosaurio, que son dinosaurios, y por tanto, no pueden estar en "mammals".
```

e.  Determina las diferencia entre los animales que no aparecen en ambos conjuntos.

```{r}
#Está hecho arriba y guardado en "notinboth
```
...
#Su representación es ligeramente distinta debido a la aparición de 3 animales más en "Animals2", pero en ambos casos se puede concluir que se sigue una distribución Gaussiana.
```
...
3.  La función **library** sin argumentos abre una ventana y muestra las librerías que han sido instaladas.
    ...
    ```{r}
libReturn <- library()
head(libReturn[[2]], n=5) #El segundo elemento de la lista contiene la matriz
length(libReturn) #Tengo un total de 3 librerías instaladas
```
...
is.na(cabbages)
lapply(cabbages, function(x){if (is.na(x)){
  rm(x)
}})
...
#Como los puntos del plot están dentro de las líneas discontinuas en ambos casos, podemos afirmar que se trata de dos distribuciones Gaussianas.
```
...
for(i in 1:nrow(cabbages)){
  if(cabbages$Cult[i] == "c39"){
    c39[[i]] <- cabbages$VitC[i]
  }else{
    c52[[i]] <- cabbages$VitC[i]
  }
}

c52 <- c52[-which(sapply(c52, is.null))]

hist(unlist(c39))
hist(unlist(c52))
#La variedad c52 contiene una cantidad ligeramente más elevada que la c39
```
...
```{r,echo=T,include=T,eval=F}
regresion <- lm(VitC ~ HeadWt, data = cabbages)
plot(cabbages$VitC, cabbages$HeadWt, xlab='VitC', ylab='HeadWt')
abline(regresion)
#No se aprecia ninguna relación entre ambas variables.
...
abline(Model1, lty=2)
abline(Model2)
#En este caso se observa claramente que no hay relación.
```
